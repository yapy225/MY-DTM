  ...
  return (
    <>
      ...
      <section className="px-4 py-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <h2 className="text-center font-serif text-3xl font-black text-dark sm:text-4xl">Nos integrations</h2>
          <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {INTEGRATIONS.map((i) => (
              <div key={i.name} className="rounded-2xl border border-border bg-white p-8 transition-all hover:-translate-y-1 hover:shadow-lg">
                ...
                {i.href && (
                  <Link href={i.href} className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-violet hover:underline">
      ...
      <section className="bg-surface px-4 py-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <h2 className="font-serif text-3xl font-black text-dark">Exemple de workflow automatise</h2>
          <div className="mt-12 space-y-4 text-left">
            {[
              ...
              <div key={i} className="flex items-start gap-4 rounded-xl border border-border bg-white p-5">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet to-gold text-sm font-bold text-white">{i + 1}</div>
                <p className="text-sm text-dark">{step}</p>
              </div>
            ))}
          </div>
          <Link href="/contact" className="mt-10 inline-flex rounded-full bg-gradient-to-r from-violet to-gold px-8 py-4 text-base font-bold text-white transition-all hover:-translate-y-1 hover:shadow-xl">
